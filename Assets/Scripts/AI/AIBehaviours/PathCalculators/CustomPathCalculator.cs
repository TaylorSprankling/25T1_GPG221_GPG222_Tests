...
public class CustomPathCalculator : MonoBehaviour, IPathingCalculator
{
    ...
    private void CalculatePath(Vector3 target)
    {
        ...
        Vector3 targetGridPosition = (target - worldScanner.transform.position - new Vector3(worldScanner.pixelSize * 0.5f, 0, worldScanner.pixelSize * 0.5f)) / worldScanner.pixelSize;
        int targetGridPosX = Mathf.RoundToInt(targetGridPosition.x);
        int targetGridPosZ = Mathf.RoundToInt(targetGridPosition.z);
        
        if (worldScanner.GridNodeReferences[targetGridPosX, targetGridPosZ] == null) return;
        
        Node targetNode = worldScanner.GridNodeReferences[targetGridPosX, targetGridPosZ];
        
        if (targetNode.IsBlocked) return; // Change this to look for closest node that isn't blocked
        
        Vector3 relativeGridPosition = (transform.position - worldScanner.transform.position - new Vector3(worldScanner.pixelSize * 0.5f, 0, worldScanner.pixelSize * 0.5f)) / worldScanner.pixelSize;
        ...
        while (open.Count > 0)
        {
            ...
            for (int xOffset = -1; xOffset < 2; xOffset++)
            {
                ...
                {
                    if (xOffset == 0 && zOffset == 0) continue; // Skip if the current node

                    if (currentNode.GridPositionX + xOffset < 0 || currentNode.GridPositionX + xOffset >= worldScanner.ScanResolution.x ||
                        currentNode.GridPositionZ + zOffset < 0 || currentNode.GridPositionZ + zOffset >= worldScanner.ScanResolution.z) continue; // Skip if outside the world scanner grid
                    ...
                    if (currentNeighbour.IsBlocked || closed.Contains(currentNeighbour)) continue; // Skip if blocked or has already been closed

                    if (currentNeighbour.HCost == -1) // If hCost hasn't been set, calculate and set it
                    {
                    ...
                    // If no parent currently set, or if this node gives a better fCost, set current node as the parent
                    if (currentNeighbour.Parent == null || currentNeighbour.HCost + gCostCalc < currentNeighbour.FCost)
                    {
        ...
        // Reset the nodes for future calculations
